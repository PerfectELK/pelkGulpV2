...
var pelk__form_send = function(form){
    ...
    try {
        if(new form__middleware($(form))){
            ...
            add_custom_value(values, $(form), 'action', 'method');
...
DG.then(function(){
    maps.each(function(index, item){
        var data = JSON.parse(item.getAttribute('data-map'));
        var map = DG.map(item, {
        ...
        for(var i = 0; i < data.markers.length; i++){
            ...
            var popup = DG.popup({'maxWidth':'500','minWidth':'150'}).setContent(marker.popup);
            DG.marker([coords[0], coords[1]]).addTo(map).bindPopup(popup);
        }
    });
});
...
function modalMiddleware(context, event){
    var item = $(context);
    if(item.data('to_modal')){
        var to_modal = item.data('to_modal');
        var modalCls = item.data('modal');
        var modal = $(modalCls);
        for(key in to_modal){
            var field = modal.find('.'+key);
            var val = to_modal[key];
            if(Array.isArray(val)){
                unslick();
    ...
    if(item.data('to_modal_form')){
        var to_form = item.data('to_modal_form');
        ...
        for(key in to_form){
            var val = to_form[key];
            if(form.find('input[name="f['+ key +']"]').length){
                form.find('input[name="f['+ key +']"]').val(val);
            }else{
                form.append('<input type="hidden" name="f['+ key +']" value="'+ val +'" >');
            }
        }
    }
}